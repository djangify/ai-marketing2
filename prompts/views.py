...
from prompt_generator.models import GeneratedPrompt
...
def import_generated_prompt(request, project_id):
    ...
    try:
        ...
        if request.headers.get('Content-Type') == 'application/json':
            ...
            prompt_id = request.POST.get('promptId')
            
        if not prompt_id:
            error_message = "Please select a generated prompt"
        ...
        generated_prompt = get_object_or_404(GeneratedPrompt, id=prompt_id, user=request.user)
        ...
        prompt = Prompt.objects.create(
            project=project,
            name=generated_prompt.name,
            prompt=generated_prompt.prompt_text,
        ...
        request.session['import_success'] = f"Successfully imported generated prompt: {generated_prompt.name}"
